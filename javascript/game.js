...
function start() {
    //as soonas we start change the button to reset // failed restart function (would of been cool tho)
    ...
    //initialise the time after the bear has been moved
...
function createBeeImg(wNum) {
    ...
    img.setAttribute("src", "../assets/bee.gif");
...
function addBee(){
    //make a bee with always increasing number
    num = bees.length + 1;
    //make a new bee
    var bee = new Bee(num);
    //display this new bee
    bee.display();
    //add it to the array
    bees.push(bee);
}
...
function updateBees() { // update loop for game
    ...
    let fails = hits.innerHTML;
    if(fails > 1000){
        var t = document.createElement("h2");
        var text = document.createTextNode("Game over");
        t.appendChild(text);
        document.getElementById("board").replaceWith(t);
        clearTimeout(lastStingTime);
    }
